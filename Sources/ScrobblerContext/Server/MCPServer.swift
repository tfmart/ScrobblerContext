...
final class MCPServer: Sendable {
    ...
    func healthCheck() async -> [String: Any] {
        var health: [String: Any] = [
            ...
            "server_version": configuration.serverVersion,
            "timestamp": Date().timeIntervalSince1970
        ]
        
        // Check Last.fm service health
        do {
            try await lastFMService.validateService()
            health["lastfm_service"] = "healthy"
        } catch {
            health["lastfm_service"] = "unhealthy"
            health["lastfm_error"] = error.localizedDescription
        }
        
        // Check authentication status
        let isAuthenticated = await lastFMService.isAuthenticated()
        health["authenticated"] = isAuthenticated
        
        // Tool counts
        let toolCount = toolExecutor.getAvailableTools().count
        health["available_tools"] = toolCount
